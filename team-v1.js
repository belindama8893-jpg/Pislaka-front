...
(() => {
  ...
  function tree(parent = 'org') { return state.units.filter(u => u.parentId === parent).map(u => `<li><div class="tv-node"><div><strong>${esc(u.name)}</strong><small>${esc(u.type)}</small></div><div class="tv-actions">${btn('Edit','edit-unit','tv-link',`data-id="${u.id}"`)}${btn('Delete','remove-unit','tv-link',`data-id="${u.id}"`)}</div></div><ul>${tree(u.id)}</ul></li>`).join(''); }
  function structure() { return `<div class="tv-card"><div class="tv-row"><div><h2>Organization structure</h2></div>${btn('+ Add Organization Unit','add-unit')}</div>${!state.units.length ? `<div class="tv-grid">${[['simple','Simple Team','One team, with room to grow.'],['branches','Teams & Branches','A branch with a team inside.'],['custom','Custom Structure','Build your own hierarchy.']].map(([id,title,desc]) => btn(`<strong>${title}</strong>`,'template','tv-template',`data-id="${id}"`)).join('')}</div>` : ''}<ul class="tv-tree"><li><div class="tv-node"><div><strong>${esc(state.org.name)}</strong></div></div><ul>${tree()}</ul></li></ul></div>`; }
  function members(accessOnly = false) { return `<div class="tv-card"><div class="tv-row"><h2>${accessOnly ? 'Roles & Access' : 'Members'}</h2>${btn('+ Add member','add-member')}</div>${state.members.map(m => `<div class="tv-member"><div class="tv-row"><div class="tv-person"><span class="tv-avatar">${esc(m.name.split(' ').map(w=>w[0]).slice(0,2).join(''))}</span><div><strong>${esc(m.name)}${m.id==='self'?' (you)':''}</strong><small>${esc(m.email)}</small>${m.unitId!=='org'?`<small>${esc(path(m.unitId))}</small>`:''}</div></div>${btn('Edit','edit-member','',`data-id="${m.id}"`)}</div><p style="margin:12px 0 4px"><span class="tv-badge">${m.access}</span> <span class="tv-badge">${m.role}</span></p>${m.role==='Manager'?`<small>${esc(scopeText(m))}</small>`:''}</div>`).join('')}${state.invites.length?`<h3 class="tv-pending-heading">Pending invitations</h3>${state.invites.map(i=>`<div class="tv-member tv-row"><div><strong>${esc(i.email)}</strong><small>${i.access} · ${i.role} · Pending</small></div><div class="tv-actions">${btn('Resend','resend','tv-link',`data-id="${i.id}"`)}${btn('Revoke','revoke','tv-link',`data-id="${i.id}"`)}</div></div>`).join('')}`:''}</div>`; }
  ...
  function review() { return `<div class="tv-card"><div class="tv-row"><h2>Review your organization</h2>${btn('Edit details','step','tv-link','data-id="1"')}</div><p>${esc(state.org.name)} · ${esc(state.org.country)} · ${esc(state.org.timezone)}</p><div class="tv-grid"><div><h3>Organization Units</h3><div class="tv-stat">${state.units.length}</div>${btn('Edit structure','step','tv-link','data-id="2"')}</div><div><h3>Members</h3><div class="tv-stat">${state.members.length}</div>${btn('Edit members','step','tv-link','data-id="3"')}</div><div><h3>Managers</h3><div class="tv-stat">${state.members.filter(m=>m.role==='Manager').length}</div></div></div></div>${members()}`; }
  ...
  root.addEventListener('click',e=>{
    ...
    if(a==='remove-unit') {
      ...
      const submit=root.querySelector('dialog [type=submit]');submit.textContent='Delete';submit.hidden=blocked;return;
    }
    if(a==='resend'){const i=state.invites.find(x=>x.id===id);i.sentAt=new Date().toISOString();save();flash('Invitation resent');return;}
    if(a==='revoke'){modal('Revoke invitation?',`<p>${esc(state.invites.find(x=>x.id===id).email)}</p>`,(f,d)=>{state.invites=state.invites.filter(x=>x.id!==id);d.close();save();render();flash('Invitation revoked');});root.querySelector('dialog [type=submit]').textContent='Revoke';return;}
